...
<head>
    ...
    <link href="{{asset('frontend/css/price-range.css')}}" rel="stylesheet">	<link href="{{asset('frontend/css/main.css')}}" rel="stylesheet">
...
<body>
	...
	<section id="slider"><!--slider-->
        ...
        <div class="item active">
            <div class="col-sm-6">
                ...
                <p>Khám phá bộ sưu tập đồng hồ sang trọng, thiết kế tinh tế dành cho phong cách của bạn.</p>
                <a href="{{ URL::to('/shop') }}" class="btn btn-default get">Mua ngay</a>
            </div>
            <div class="col-sm-6">
                <img src="{{ URL::to('upload/product/citizen-promaster.jpg') }}" class="img-responsive" alt="Đồng hồ cao cấp XWatch" />
            </div>
        </div>
        <div class="item">
            <div class="col-sm-6">
                <h1><span>X</span>WATCH</h1>
                <h2>Thiết kế thời thượng</h2>
                <p>Nâng tầm phong cách với đồng hồ hiện đại, phù hợp mọi dịp.</p>
                <a href="{{ URL::to('/shop') }}" class="btn btn-default get">Mua ngay</a>
            </div>
            <div class="col-sm-6">
                <img src="{{ URL::to('upload/product/citizen-promaster.jpg') }}" class="img-responsive" alt="Đồng hồ thời thượng XWatch" />
            </div>
        </div>
        <div class="item">
            <div class="col-sm-6">
                ...
                <p>Nhận ngay giảm giá lên đến 30% cho các mẫu đồng hồ mới nhất.</p>
                <a href="{{ URL::to('/shop') }}" class="btn btn-default get">Mua ngay</a>
            </div>
            <div class="col-sm-6">
                <img src="{{ URL::to('upload/product/citizen-promaster.jpg') }}" class="img-responsive" alt="Đồng hồ ưu đãi XWatch" />
            </div>
        </div>
    ...
    <script src="{{asset('frontend/js/jquery.prettyPhoto.js')}}"></script>    <script src="{{asset('frontend/js/main.js')}}"></script>
	...
	<script>
			...
			if(cityId == 0 || provinceId == 0 || wardId == 0) {
				...
				$.ajax({
                    ...
                    success: function (response) {
                    	// Update phí vận chuyển trong DOM
                    	...
                    	// Restore button
                    	$button.prop('disabled', false).text('Tính phí vận chuyển');
                    },
                    error: function() {
		...
		// Function để update hiển thị phí vận chuyển
		function updateShippingFeeDisplay() {
			// Reload phần hiển thị tổng tiền với AJAX
			$.ajax({
				url: '{{url::to("/get-cart-summary")}}',
				method: 'GET',
				success: function(data) {
					// Update phần hiển thị tổng tiền nếu có
					if($('.cart-summary').length > 0) {
						$('.cart-summary').html(data);
					}
					// Update hidden input phí vận chuyển
					if($('.fee_shipping').length > 0) {
						$.get('{{url::to("/get-current-fee")}}', function(fee) {
							$('.fee_shipping').val(fee);
						});
					}
				}
			});
